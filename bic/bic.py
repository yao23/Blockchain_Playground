...
def get_balance():
    ...
    s_b = 0.0
    balances = spot.get('snapshotVos')[-1].get('data').get('balances')
    for b in balances:
        print('asset: ' + b.get('asset'))
        price = 1.0
        if b.get('asset') != 'USDT':
            asset = client.get_symbol_ticker(symbol=b.get('asset') + 'USDT')
            print(asset)
            price = float(asset.get('price'))
        # asset = client.get_ticker(b.get('asset'))
        # assets = client.get_symbol_ticker()
        # for a in assets:
            # print str(a)[1:-1] 
        s_b = s_b + price * float(b.get('free'))
    print('s_b: ' + str(s_b) + ', s_balance: ' + s_balance)        
    # print('s_balance: ' + s_balance)        
    ...
    for k, v in future.items():
        print(k, v)
    f_balance = future.get('snapshotVos')[-1].get('data').get('assets')[-1].get('marginBalance')
    print('f_balance: ' + f_balance)        

    # # get all symbol prices
    # prices = client.get_all_tickers()
    # print('prices: ' + str(prices)[1:-1] )
    add_to_gs(str(s_b), f_balance)

def add_to_gs(s_balance, f_balance):
    try:
